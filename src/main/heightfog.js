import * as THREE from 'three'

export class HeightFog {

    constructor(scene, options = {}) {

        this.scene = scene

        this.enabled = options.enabled ?? true
        this.radius = options.radius ?? 500.0

        this.color = new THREE.Color(options.color ?? 0xE3EAF2)
        this.fogTop = options.fogTop ?? 4.0
        this.fogDepth = options.fogDepth ?? 6.0
        this.opacity = options.opacity ?? 0.8
        this.exponent = options.exponent ?? 1.2

        this.noiseScale = options.noiseScale ?? 0.08
        this.noiseStrength = options.noiseStrength ?? 0.12
        this.noiseOctaves = Math.max(1, Math.min(8, options.noiseOctaves ?? 4))

        this.innerRadius = options.innerRadius ?? 2.0
        this.innerFade = options.innerFade ?? 3.0

        this.windDir = new THREE.Vector2(options.windDirX ?? 0.0, options.windDirZ ?? 1.0).normalize()

        this.windSpeed = options.windSpeed ?? 0.8
        this.verticalBillow = options.verticalBillow ?? 0.35

        this.materials = new Set()
        this._time = 0

        this.patchScene()

    }

    patchScene() {
        this.scene.traverse(obj=>{
        if(obj.isMesh) this.patchMaterial(obj)
        })
    }

    patchMaterial(mesh){

        const mats = Array.isArray(mesh.material) ? mesh.material : [mesh.material]

        for(const mat of mats){

            if(!mat || mat.userData._heightFogPatched) continue

            mat.userData._heightFogPatched = true

            const orig = mat.onBeforeCompile

            mat.onBeforeCompile = shader=>{
                    if(orig) orig(shader)

                    shader.uniforms.uFogEnabled = { value: this.enabled ? 1 : 0 }
                    shader.uniforms.uFogRadius = { value:this.radius }
                    shader.uniforms.uFogColor = { value:this.color.clone() }
                    shader.uniforms.uFogTop = { value:this.fogTop }
                    shader.uniforms.uFogDepth = { value:this.fogDepth }
                    shader.uniforms.uFogOpacity = { value:this.opacity }
                    shader.uniforms.uFogExponent = { value:this.exponent }
                    shader.uniforms.uInnerRadius = { value: this.innerRadius }
                    shader.uniforms.uInnerFade = { value: this.innerFade }
                    shader.uniforms.uNoiseScale = { value:this.noiseScale }
                    shader.uniforms.uNoiseStrength = { value:this.noiseStrength }
                    shader.uniforms.uNoiseOctaves = { value:this.noiseOctaves }

                    shader.uniforms.uWindDir = {
                    value:new THREE.Vector2(this.windDir.x,this.windDir.y)
                }

                shader.uniforms.uWindSpeed = { value:this.windSpeed }
                shader.uniforms.uVerticalBillow = { value:this.verticalBillow }
                shader.uniforms.uTime = { value:this._time }

                mat.userData.heightFogUniforms = shader.uniforms

                shader.vertexShader =
                "varying vec3 vWorldPos;\n" + shader.vertexShader

                shader.vertexShader = shader.vertexShader.replace(
                    "#include <worldpos_vertex>",
                    `
                    #include <worldpos_vertex>
                    #ifdef USE_INSTANCING
                    vWorldPos = (modelMatrix * instanceMatrix * vec4(position,1.0)).xyz;
                    #else
                    vWorldPos = (modelMatrix * vec4(position,1.0)).xyz;
                    #endif
                    `
                )

                const NOISE = `
                    vec3 mod289(vec3 x){ return x-floor(x*(1.0/289.0))*289.0; }
                    vec4 mod289(vec4 x){ return x-floor(x*(1.0/289.0))*289.0; }
                    vec4 permute(vec4 x){ return mod289(((x*34.0)+1.0)*x); }
                    vec4 taylorInvSqrt(vec4 r){ return 1.79284291400159-0.85373472095314*r; }

                    float snoise(vec3 v){
                    const vec2 C=vec2(1.0/6.0,1.0/3.0);
                    const vec4 D=vec4(0.0,0.5,1.0,2.0);
                    vec3 i=floor(v+dot(v,C.yyy));
                    vec3 x0=v-i+dot(i,C.xxx);
                    vec3 g=step(x0.yzx,x0.xyz);
                    vec3 l=1.0-g;
                    vec3 i1=min(g.xyz,l.zxy);
                    vec3 i2=max(g.xyz,l.zxy);
                    vec3 x1=x0-i1+C.xxx;
                    vec3 x2=x0-i2+C.yyy;
                    vec3 x3=x0-D.yyy;
                    i=mod289(i);
                    vec4 p=permute(permute(permute(
                    i.z+vec4(0.0,i1.z,i2.z,1.0))
                    +i.y+vec4(0.0,i1.y,i2.y,1.0))
                    +i.x+vec4(0.0,i1.x,i2.x,1.0));
                    vec3 ns=C.x*D.wyz-D.xzx;
                    vec4 j=p-49.0*floor(p*ns.z*ns.z);
                    vec4 x_=floor(j*ns.z);
                    vec4 y_=floor(j-7.0*x_);
                    vec4 x=x_*ns.x+ns.yyyy;
                    vec4 y=y_*ns.x+ns.yyyy;
                    vec4 h=1.0-abs(x)-abs(y);
                    vec4 b0=vec4(x.xy,y.xy);
                    vec4 b1=vec4(x.zw,y.zw);
                    vec4 s0=floor(b0)*2.0+1.0;
                    vec4 s1=floor(b1)*2.0+1.0;
                    vec4 sh=-step(h,vec4(0.0));
                    vec4 a0=b0.xzyw+s0.xzyw*sh.xxyy;
                    vec4 a1=b1.xzyw+s1.xzyw*sh.zzww;
                    vec3 p0=vec3(a0.xy,h.x);
                    vec3 p1=vec3(a0.zw,h.y);
                    vec3 p2=vec3(a1.xy,h.z);
                    vec3 p3=vec3(a1.zw,h.w);
                    vec4 norm=taylorInvSqrt(vec4(dot(p0,p0),dot(p1,p1),dot(p2,p2),dot(p3,p3)));
                    p0*=norm.x; p1*=norm.y; p2*=norm.z; p3*=norm.w;
                    vec4 m=max(0.5-vec4(dot(x0,x0),dot(x1,x1),dot(x2,x2),dot(x3,x3)),0.0);
                    m=m*m;
                    return 105.0*dot(m*m,vec4(dot(p0,x0),dot(p1,x1),dot(p2,x2),dot(p3,x3)));
                    }

                    float fbm(vec3 p,int oct){
                    float v=0.0;
                    float a=0.5;
                    float f=1.0;
                    for(int i=0;i<6;i++){
                    if(i>=oct) break;
                    v+=a*snoise(p*f);
                    f*=2.0;
                    a*=0.5;
                    }
                    return clamp(v*0.5+0.5,0.0,1.0);
                    }
                `

                shader.fragmentShader = `
                    varying vec3 vWorldPos;

                    uniform int uFogEnabled;
                    uniform float uFogRadius;
                    uniform vec3 uFogColor;
                    uniform float uFogTop;
                    uniform float uFogDepth;
                    uniform float uFogOpacity;
                    uniform float uFogExponent;

                    uniform float uInnerRadius;
                    uniform float uInnerFade;

                    uniform float uNoiseScale;
                    uniform float uNoiseStrength;
                    uniform int uNoiseOctaves;

                    uniform vec2 uWindDir;
                    uniform float uWindSpeed;
                    uniform float uVerticalBillow;
                    uniform float uTime;

                    ${NOISE}
                    ` + shader.fragmentShader

                    shader.fragmentShader = shader.fragmentShader.replace(
                    "#include <fog_fragment>",
                `
                    #include <fog_fragment>
                    if(uFogEnabled == 0) return;

                    if(vWorldPos.y<uFogTop){

                        vec2 camXZ=cameraPosition.xz;
                        vec2 posXZ=vWorldPos.xz;

                        float distXZ=distance(camXZ,posXZ);


                        if(uFogRadius>0.0 && distXZ<=uFogRadius){

                            float normalized=clamp((uFogTop-vWorldPos.y)/max(0.0001,uFogDepth),0.0,1.0);

                            vec3 noisePos=vWorldPos*uNoiseScale;

                            vec3 wind=vec3(uWindDir.x,0.0,uWindDir.y)
                            *uWindSpeed*uTime*(uNoiseScale*2.5);

                            wind.y+=sin(uTime*0.07+noisePos.x*0.5+noisePos.z*0.5)
                            *uVerticalBillow;

                            noisePos+=wind;

                            float nLarge=fbm(noisePos*0.35,3);
                            float nDetail=fbm(noisePos*1.5,uNoiseOctaves);

                            float nRaw=mix(nLarge,nDetail,0.35);
                            float nMix=mix(0.5,nRaw,uNoiseStrength);

                            float distanceFade=smoothstep(0.0,uFogRadius,distXZ);
                            nMix=mix(nMix,nLarge,distanceFade);

                            float heightNoise=mix(0.85,1.15,nMix);
                            float heightAtten=pow(normalized,uFogExponent);
                            heightAtten*=heightNoise;

                            if(heightAtten>0.0){

                                float noiseMod=mix(0.8,1.2,nMix);
                                noiseMod=smoothstep(0.1,1.0,noiseMod);

                                float outerFade=1.0-smoothstep(uFogRadius*0.7,uFogRadius,distXZ);

                                float innerFade=smoothstep(uInnerRadius,uInnerRadius+uInnerFade,distXZ);

                                float radiusFactor = outerFade * innerFade;

                                float fogFactor=heightAtten*noiseMod;
                                fogFactor*=uFogOpacity*radiusFactor;
                                fogFactor=pow(fogFactor,1.2);

                                vec3 foggedColor = mix(
                                    gl_FragColor.rgb,
                                    uFogColor * 0.92,
                                    fogFactor
                                );

                                float horizon = smoothstep(uFogRadius * 0.3, uFogRadius, distXZ);
                                foggedColor = mix(foggedColor, uFogColor, horizon * 0.6);

                                gl_FragColor.rgb = foggedColor;

                            }

                        }

                    }
                    `
                )

            }

            mat.needsUpdate = true
            this.materials.add(mat)

        }

    }

    updateTime(t){
        this._time = t
        for(const m of this.materials){
            const u = m.userData.heightFogUniforms
            if(!u) continue

            u.uFogEnabled.value = this.enabled ? 1 : 0
            u.uTime.value = t
            u.uFogRadius.value = this.radius
            u.uFogColor.value.copy(this.color)
            u.uFogTop.value = this.fogTop
            u.uFogDepth.value = this.fogDepth
            u.uFogOpacity.value = this.opacity
            u.uFogExponent.value = this.exponent
            u.uNoiseScale.value = this.noiseScale
            u.uNoiseStrength.value = this.noiseStrength
            u.uNoiseOctaves.value = this.noiseOctaves
            u.uWindDir.value.set(this.windDir.x,this.windDir.y)
            u.uWindSpeed.value = this.windSpeed
            u.uVerticalBillow.value = this.verticalBillow
            u.uInnerRadius.value = this.innerRadius
            u.uInnerFade.value = this.innerFade
        }
    }

    setEnabled(v){ this.enabled = v }
    setRadius(v){ this.radius=v }
    setFogTop(v){ this.fogTop=v }
    setFogDepth(v){ this.fogDepth=v }
    setColor(c){ this.color.set(c) }
    setOpacity(v){ this.opacity=v }
    setExponent(v){ this.exponent=v }
    setNoiseScale(v){ this.noiseScale=v }
    setNoiseStrength(v){ this.noiseStrength=v }
    setNoiseOctaves(v){ this.noiseOctaves=v }
    setWind(x,z){ this.windDir.set(x,z).normalize() }
    setWindSpeed(v){ this.windSpeed=v }
    setVerticalBillow(v){ this.verticalBillow=v }
    setInnerRadius(v){ this.innerRadius = v }
    setInnerFade(v){ this.innerFade = v }

}